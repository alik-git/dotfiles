...
_work_mnv1_can_use_ssh_auth_sock() {
...
_work_mnv1_set_stable_ssh_auth_sock() {
    ...
    if [ "$source_sock" = "$stable_sock" ]; then
        _work_mnv1_can_use_ssh_auth_sock "$stable_sock" || return 1
        export SSH_AUTH_SOCK="$stable_sock"
        return 0
    fi
    _work_mnv1_is_shpool_ssh_auth_sock "$source_sock" && return 1
    _work_mnv1_can_use_ssh_auth_sock "$source_sock" || return 1
    mkdir -p "$HOME/.ssh"
...
_work_mnv1_find_forwarded_ssh_auth_sock() {
    ...
    for candidate_sock in /tmp/ssh-*/agent.*; do
        _work_mnv1_can_use_ssh_auth_sock "$candidate_sock" || continue
        printf '%s\n' "$candidate_sock"
        return 0
    done
    return 1
...
_work_mnv1_refresh_stable_ssh_auth_sock() {
    if [ -n "${SSH_AUTH_SOCK:-}" ] && \
        ! _work_mnv1_is_shpool_ssh_auth_sock "$SSH_AUTH_SOCK" && \
        _work_mnv1_can_use_ssh_auth_sock "$SSH_AUTH_SOCK"; then
        _work_mnv1_set_stable_ssh_auth_sock "$SSH_AUTH_SOCK" && return 0
    fi
    _work_mnv1_can_use_ssh_auth_sock "$HOME/.ssh/ssh_auth_sock" || return 1
...
fixssha() {
    ...
    if _work_mnv1_can_use_ssh_auth_sock "$HOME/.ssh/ssh_auth_sock"; then
        export SSH_AUTH_SOCK="$HOME/.ssh/ssh_auth_sock"
...
elif _work_mnv1_can_use_ssh_auth_sock "$HOME/.ssh/ssh_auth_sock"; then
    export SSH_AUTH_SOCK="$HOME/.ssh/ssh_auth_sock"
fi
